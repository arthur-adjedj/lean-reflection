...
instance : EmptyCollection Dir := ⟨.none⟩
notation "≤ₐ" => Dir.leq
notation "≥ₐ" => Dir.geq
notation "=ₐ" => Dir.eq
...
inductive Varₚ (d : Dir): List Dir → Type (u+1)
| vz : d ≤ d' → Varₚ d [d']
...
inductive Tmₛ.{u}: Conₛ.{u} l → Tyₛ.{u} l → Dir → Type (u+1)
...
/--Example of a constructor that cannot be valid.
   `V_invalid : A → (A → List A) → List A`
   `A` appears in a contravariant position despite being marked as covariant.
   As such, the `sorry` cannot be proven.
-/
  def V_invalid : Tyₚ Vₛ  :=
    PFunc (Tmₛ.varParam (.vz ⟨⟩)) <| -- A →
...
def List.Telescope.run (l : List Dir) (params : List (Type u))
(h : l.length = params.length) (tel : l.Telescope A): A :=
match l,params with
  | .nil,.nil => tel
  | .cons _ tl₁,.cons T tl₂ => List.Telescope.run tl₁ tl₂ (by simp_all) (tel T)
